# -*- coding: utf-8 -*-
"""
Created on Thu Oct 30 18:23:05 2025
...
def parameter_sweep(param_name, param_values, n_runs=20, base_seed=1000,
                    run_simulation_func=run_simulation, save_prefix="sweep"):
    """
    param_name: str (ex: "theta_global", "lam", "T_param")
    param_values: list of values to test
    n_runs: number of runs per parameter value
    base_seed: integer seed offset; seeds used = base_seed + run_idx
    run_simulation_func: function that executes 1 run and returns dict (same as run_simulation)
    save_prefix: prefix for output files
    """
    summary_rows = []
    all_series = {}  # optionnel : stocker séries moyennes par param
    ...
    for val in param_values:
        ...
        for r in range(n_runs):
            s = base_seed + r
            # Construire kwargs dynamiques : on part du principe que run_simulation accepte ces paramètres
            kwargs = dict(
                N=N, network_type=network_type, z=z, L=L, lam=lam,
                R=R, T_param=T_param, S=S, P=P,
                generations=generations, seed=s,
                theta_global=theta_global, initial_coop_frac=initial_coop_frac
            )

            # remplacer le paramètre testé
            kwargs[param_name] = val

            res = run_simulation_func(**kwargs)
            results_runs.append(res)
            series_coop.append(res["history_coop"])
            series_vig.append(res["history_vig"])
            series_avgT.append(res["history_avg_T"])

            print(f"  run {r+1}/{n_runs} seed={s} final_coop={res['final_coop']:.3f}")

        # calculs statistiques (valeurs finales)
        finals_coop = np.array([r["final_coop"] for r in results_runs])
        finals_vig  = np.array([r["final_vig"]  for r in results_runs])

        mean_coop = finals_coop.mean()
        std_coop  = finals_coop.std(ddof=1)
        med_coop  = np.median(finals_coop)
        ci95_coop = 1.96 * std_coop / np.sqrt(n_runs)  # approx 95% (pour n raisonnable)

        mean_vig = finals_vig.mean()
        std_vig  = finals_vig.std(ddof=1)
        ci95_vig = 1.96 * std_vig / np.sqrt(n_runs)

        summary_rows.append([param_name, val, n_runs,
                             mean_coop, std_coop, med_coop, ci95_coop,
                             mean_vig, std_vig, ci95_vig])

        # moyenne temporelle
        series_coop = np.array(series_coop)  # shape (n_runs, generations)
        mean_series_coop = series_coop.mean(axis=0)
        all_series[val] = {
            "mean_coop": mean_series_coop,
            "mean_vig": np.array(series_vig).mean(axis=0),
            "mean_avgT": np.array(series_avgT).mean(axis=0)
        }

    # Sauvegarde résumé CSV
    csvname = f"{save_prefix}_{param_name}_summary.csv"
    with open(csvname, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["param", "value", "n_runs",
                    "mean_coop", "std_coop", "median_coop", "ci95_coop",
                    "mean_vig", "std_vig", "ci95_vig"])
        for row in summary_rows:
            w.writerow(row)
    print(f"> Résumé sauvegardé: {csvname}")

    return summary_rows, all_series
...
if __name__ == "__main__":
    ...
    # === EXPLORACIÓN: variación de θ (umbral de vigilancia) ===
    thetas = np.linspace(0.0, 1.0, 11)  # 11 valores entre 0 y 1
    summary, series = parameter_sweep("theta_global", thetas, n_runs=20, base_seed=2000, save_prefix="theta_sweep")

import pandas as pd

df = pd.read_csv("theta_sweep_theta_global_summary.csv")
plt.errorbar(df["value"], df["mean_coop"], fmt='o-', label="Coop media")
plt.errorbar(df["value"], df["mean_vig"], fmt='s--', label="Vigi media")
plt.xlabel("θ (umbral de vigilancia)")
plt.ylabel("Fraction moyenne final")
plt.legend()
plt.grid(alpha=0.3)
plt.show()
